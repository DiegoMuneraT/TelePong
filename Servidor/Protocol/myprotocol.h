...
void receiveTextFromClient(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len, char *text)
{
...
void REQUEST(char *header, char value)
{
    if(header[0] == 'G' && header[1] == 'E' && header[2] == 'T')
    {
        ...
        receiveTextFromClient(sockfd, &client_addr, addr_len, buffer);
